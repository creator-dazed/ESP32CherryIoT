const int spkrPin = 3; //3:ConnectorA 4:ConnectorB

#define DO 261.6
#define _DO 277.18
#define RE 293.665
#define _RE 311.127
#define MI 329.63
#define FA 349.228
#define _FA 369.994
#define SO 391.995
#define _SO 415.305
#define RA 440
#define _RA 466.164
#define TI 493.883
#define octDO 523.251

void playmusic(){
  // --- 1. きらきら光る お空の星よ ---
  ledcWriteTone(spkrPin, DO); delay(400);
  ledcWriteTone(spkrPin, DO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, RA); delay(400);
  ledcWriteTone(spkrPin, RA); delay(400);
  ledcWriteTone(spkrPin, SO); delay(800);

  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, RE); delay(400);
  ledcWriteTone(spkrPin, RE); delay(400);
  ledcWriteTone(spkrPin, DO); delay(800);

  // --- 2. まばたきしては みんなを見てる ---
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, RE); delay(800);

  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, RE); delay(800);

  // --- 3. きらきら光る お空の星よ（リフレイン） ---
  ledcWriteTone(spkrPin, DO); delay(400);
  ledcWriteTone(spkrPin, DO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, SO); delay(400);
  ledcWriteTone(spkrPin, RA); delay(400);
  ledcWriteTone(spkrPin, RA); delay(400);
  ledcWriteTone(spkrPin, SO); delay(800);

  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, FA); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, MI); delay(400);
  ledcWriteTone(spkrPin, RE); delay(400);
  ledcWriteTone(spkrPin, RE); delay(400);
  ledcWriteTone(spkrPin, DO); delay(800);

  ledcWriteTone(spkrPin, 0); // no sound
}

void setup() {
  pinMode(spkrPin, OUTPUT);
  ledcAttach(spkrPin, 12000, 8); 
}

void loop() {
  playmusic();
  delay(3000);
}